...
export class OnboardingService {
  ...
  upsertRetentionPolicy(tenantId: string, actorId: string, dto: UpsertOnboardingRetentionPolicyDto) { return this.prisma.onboardingRetentionPolicy.upsert({ where: { tenantId_countryCode_documentCategory: { tenantId, countryCode: dto.countryCode.trim().toUpperCase(), documentCategory: dto.documentCategory.trim().toUpperCase() } }, update: { retentionDays: dto.retentionDays, legalBasis: dto.legalBasis?.trim(), isActive: true, legalReviewStatus: 'DRAFT', reviewedAt: null, reviewedById: null }, create: { tenantId, countryCode: dto.countryCode.trim().toUpperCase(), documentCategory: dto.documentCategory.trim().toUpperCase(), retentionDays: dto.retentionDays, legalBasis: dto.legalBasis?.trim(), createdById: actorId } }); }
  async approveRetentionPolicy(tenantId: string, actorId: string, id: string, _dto: ApproveOnboardingRetentionPolicyDto) { const policy = await this.prisma.onboardingRetentionPolicy.findFirst({ where: { id, tenantId } }); if (!policy) throw new NotFoundException('Retention policy not found'); if (policy.createdById === actorId) throw new ForbiddenException('Otra persona debe aprobar la regla de retención'); return this.prisma.onboardingRetentionPolicy.update({ where: { id }, data: { legalReviewStatus: 'APPROVED', reviewedById: actorId, reviewedAt: new Date() } }); }
