...
export class SalesImportController {
  ...
  @Get('template') @RequirePermissions('inventory.sales_import.view') template(@Res() response: Response) { response.setHeader('Content-Type', 'text/csv; charset=utf-8'); response.setHeader('Content-Disposition', 'attachment; filename="ventas-importacion.csv"'); return response.send('external_sale_id,external_product_code,external_product_name,quantity,sold_at\nVENTA-001,HAMB-001,Hamburguesa,1,2026-08-23T12:00:00Z\n'); }
  @Post('upload') @RequirePermissions('inventory.sales_import.create') @UseInterceptors(FileInterceptor('file', { limits: { fileSize: Number(process.env.SALES_IMPORT_MAX_BYTES ?? 25 * 1024 * 1024), files: 1 } })) upload(@Req() r: RequestWithUser, @UploadedFile() file: Express.Multer.File, @Query('branchId') branchId: string, @Query('warehouseId') warehouseId: string) { return this.service.upload(r.tenant!.id, r.user.sub, branchId, warehouseId, file); }
  @Get(':id') @RequirePermissions('inventory.sales_import.view') summary(@Req() r: RequestWithUser, @Param('id') id: string) { return this.service.summary(r.tenant!.id, id); }
  @Post(':id/validate') @RequirePermissions('inventory.sales_import.validate') validate(@Req() r: RequestWithUser, @Param('id') id: string) { return this.service.validate(r.tenant!.id, id); }
  @Post(':id/process') @RequirePermissions('inventory.sales_import.process') process(@Req() r: RequestWithUser, @Param('id') id: string) { return this.service.process(r.tenant!.id, r.user.sub, id); }
  @Post(':id/cancel') @RequirePermissions('inventory.sales_import.reverse') cancel(@Req() r: RequestWithUser, @Param('id') id: string) { return this.service.cancel(r.tenant!.id, id); }
  @Post('mappings') @RequirePermissions('inventory.sales_import.map') mapping(@Req() r: RequestWithUser, @Body() dto: any) { return this.service.mapProduct(r.tenant!.id, r.user.sub, dto); }
}
