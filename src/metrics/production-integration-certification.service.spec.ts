...
describe("ProductionIntegrationCertificationService", () => {
  ...
  it("reports incomplete production configuration without exposing secrets", async () => {
    ...
    const service = new ProductionIntegrationCertificationService({} as never, {} as never);
  ...
  it("aggregates successful active probes", async () => {
    ...
    jest.spyOn(service as any, "certifyRedis").mockResolvedValue(pass("redis-queues"));
    jest.spyOn(service as any, "certifyTrainingWebhooks").mockResolvedValue(pass("training-webhooks"));
    ...
    expect(report.summary).toEqual({ passed: 9, warnings: 0, failed: 0, skipped: 0 });
  });
});
